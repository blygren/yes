...
checklistBtn.style.cssText = "position: fixed; top: 10px; left: 10px; z-index: 1000; padding: 12px 20px; font-size: 16px; cursor: pointer; background: white; border: 2px solid black; border-radius: 8px; font-family: sans-serif; touch-action: manipulation; -webkit-tap-highlight-color: transparent;";
document.body.appendChild(checklistBtn);

// Modal
const checklistModal = document.createElement('div');
checklistModal.style.cssText = "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: white; color: black; z-index: 1001; padding: 15px; overflow-y: auto; display: none; font-family: sans-serif; -webkit-overflow-scrolling: touch;";
document.body.appendChild(checklistModal);

// Close Button
const closeBtn = document.createElement('button');
closeBtn.textContent = "Close";
closeBtn.style.cssText = "float: right; margin-bottom: 10px; cursor: pointer; padding: 10px 20px; font-size: 16px; border: 2px solid black; background: white; border-radius: 8px; touch-action: manipulation; -webkit-tap-highlight-color: transparent;";
...
wipeBtn.style.cssText = "float: right; margin-bottom: 10px; margin-right: 10px; cursor: pointer; padding: 10px 20px; font-size: 16px; background-color: #ff4444; color: white; border: 2px solid #cc0000; border-radius: 8px; touch-action: manipulation; -webkit-tap-highlight-color: transparent;";
wipeBtn.onclick = () => {
...
function renderChecklist() {
    ...
    Object.keys(categories).forEach(catName => {
        ...
        categories[catName].forEach(zoneItem => {
            ...
            if (isVisited) {
                item.ontouchstart = () => item.style.transform = "scale(1.05)";
                item.ontouchend = () => {
                    item.style.transform = "scale(1)";
                    showZoneDetails(zoneItem, count);
                };
...
// Wheel Jacking Logic (keep existing for desktop)
