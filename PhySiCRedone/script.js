    ...
    state.objects = allBodies
        ...
        .map(body => {
            return {
                ...
                isRainbowName: body.isRainbowName // <--- added
            };
        });
...
function loadState(state) {
    ...
    const newBodies = state.objects.map(obj => {
        ...
        body.isRainbowName = obj.isRainbowName || false; // <--- restore
        return body;
    });
...
function spawnBall(x, y) {
    ...
    if (namesEnabled) {
        // 1 in 30 chance for rainbow name
        const isRainbow = Math.random() < (1 / 30);
        ball.customName = namePool[Math.floor(Math.random() * namePool.length)];
        if (isRainbow) {
            ball.isRainbowName = true;
        }
    }
